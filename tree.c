...
// Recursive helper: build a tree from index entries that share a common prefix.
// `entries`  - array of index entries
// `count`    - number of entries in the array
// `prefix`   - directory prefix being processed (e.g. "src/")
// `id_out`   - where to store the resulting tree's ObjectID
//
// Returns 0 on success, -1 on error.
static int write_tree_level(IndexEntry *entries, int count, const char *prefix, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        const char *rel = e->path + prefix_len; // path relative to current level

        // Find the first '/' after the prefix — determines if this is a dir entry
        const char *slash = strchr(rel, '/');

        if (!slash) {
            // This is a direct file in the current directory — make a blob entry
            ...
            // This entry belongs to a subdirectory — gather all entries in that subdir
            ...
            ObjectID sub_id;
            if (write_tree_level(&entries[sub_start], sub_count, sub_prefix, &sub_id) != 0)
                return -1;

            // Add a directory entry in the current tree
            if (tree.count >= MAX_TREE_ENTRIES) return -1;
            TreeEntry *te = &tree.entries[tree.count++];
            te->mode = MODE_DIR;
            te->hash = sub_id;
...
// Comparator for sorting index entries by path (for tree_from_index)
...
int tree_from_index(ObjectID *id_out) {
    ...
    // Sort entries by path so subdirectory grouping works with a linear scan
    qsort(index.entries, index.count, sizeof(IndexEntry), compare_index_entries_by_path);

    // Build from the root (empty prefix)
